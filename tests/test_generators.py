...
def test_filter_by_currency(list_dicts_with_transactions: list[dict[str, Any]], currency_filter_1: str) -> None:
    """Тест, проверяющий, что функция `filter_by_currency` корректно фильтрует транзакции по заданной валюте"""
...
def test_filter_by_currency_zero_list(list_dict_zero_for_test: list, currency_filter_1: str) -> None:
    """Тест, проверяющий, что функция `filter_by_currency` корректно фильтрует транзакции по заданной валюте,
    когда передается пустой список"""
...
def test_transaction_descriptions_zero(list_dict_zero_for_test: list) -> None:
    """Тест, проверяющий, что функция `transaction_descriptions` возвращает корректнjое сообщение при пустом списке"""
...
def test_transaction_descriptions_incomplete_data(list_dict_incomplete_data_for_test: list) -> None:
    """Тест, проверяющий, что функция `transaction_descriptions` корректно обрабатывает список с неполными данными"""
...
def test_card_number_generator_incorrect_input() -> None:
    """Тест, проверяющий, что генератор 'card_number_generator' правильно обрабатывает некорректный ввод данных"""
